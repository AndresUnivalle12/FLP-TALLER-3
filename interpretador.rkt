...
;;Andres Felipe Lopez 2128542-3743
;;URL Github: https://github.com/AndresUnivalle12/FLP-TALLER-3
...
(scan&parse "declarar (@x=5; @a=procedimiento (@x)haga recursivo (@fact(@x) = Si @x entonces (@x * evaluar @fact(sub1(@x)) finEval) sino 1 finSI) { evaluar @fact(@x) finEval} finProc) { evaluar @a(@x) finEval}")
(scan&parse "declarar (@x=10; @a=procedimiento (@x)haga recursivo (@fact(@x) = Si @x entonces (@x * evaluar @fact(sub1(@x)) finEval) sino 1 finSI) { evaluar @fact(@x) finEval} finProc) { evaluar @a(@x) finEval}")
;Punto c) @Suma:
...
;; Decorador 1 e)
(scan&parse "recursivo (@integrantes() = \"AndresLopez\";@saludar(@func) = (\"hola\" concat evaluar @func() finEval);@decorate()=evaluar@saludar(@integrantes) finEval ) { evaluar @decorate() finEval }")
;; Decorador 2 f)
(scan&parse "recursivo (@integrantes() = \"AndresLopez\";@saludar(@func) = (\"hola\" concat evaluar @func() finEval);@decorate(@str)= (evaluar@saludar(@integrantes) finEval concat @str)) { evaluar @decorate(\"adios\") finEval }")
(interpretador)